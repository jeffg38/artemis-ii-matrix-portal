...
#
# WiFi credentials + Adafruit IO → /CIRCUITPY/settings.toml:
#   CIRCUITPY_WIFI_SSID = "your_ssid"
#   CIRCUITPY_WIFI_PASSWORD = "your_password"
#   ADAFRUIT_AIO_USERNAME = "your_username"
#   ADAFRUIT_AIO_KEY = "your_key"
#
# Adafruit IO feeds (create these at io.adafruit.com):
#   artemis-altitude   (numeric, miles from Earth)
...
def push_to_io(alt_miles, speed, met_secs):
    """Push telemetry values to Adafruit IO feeds. Fails silently."""
    try:
        gc.collect()
        if alt_miles > 0:
            matrixportal.network.push_to_io("artemis-altitude", alt_miles)
        if speed > 0:
            matrixportal.network.push_to_io("artemis-speed", speed)
        if met_secs > 0:
            matrixportal.network.push_to_io("artemis-met", met_secs)
        print("Logged to Adafruit IO")
    except Exception as e:
        print("IO log error:", e)
...
def fetch_and_format():
    ...
    try:
        ...
        speed_str = f"{speed:,}" if speed > 0 else "---"

        # MET from timestamp
        met_secs = 0
        met_str = "---"
        if ts:
            met_secs = int(float(ts) - 1775082240)
            if 0 < met_secs < 864000:
                met_str = f"T+{met_secs//3600}h{(met_secs%3600)//60:02}m"
            else:
                met_secs = 0
        ...
        # ── Log to Adafruit IO ──
        push_to_io(alt_miles, speed, met_secs)
